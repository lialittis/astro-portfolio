...
import { ColDef, GridOptions, GridApi, createGrid } from "ag-grid-community";
...
const AGGridTable: React.FC = () => {
  ...
  useEffect(() => {
    if (gridRef.current) {
      gridApi = createGrid(gridRef.current, {
    ...
    return () => {
      gridApi?.destroy(); // Cleanup on unmount
    };
  }, []);

  return <div ref={gridRef} className="ag-theme-alpine" style={{ width: "100%", height: "500px" }} />;
};
